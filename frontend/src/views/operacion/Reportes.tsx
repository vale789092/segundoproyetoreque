...
export default function Reportes() {
  ...
  // 1) Derivar modo desde la URL (usa includes para tolerar barras finales o query)
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* 🔑 key={mode} obliga a remontar Tabs cuando cambia la URL
          onActiveTabChange nos da el índice del tab clickeado, navegamos. */}
      <Tabs
        key={mode}
      ...
      <Card>
        <div className="flex flex-wrap items-end gap-4">
          ...
          {/* Botones “stub” para cuando conectes los endpoints */}
          {mode==="resumen" && (
            <>
              <Button disabled>Descargar XLSX</Button>
              <Button color="light" disabled>Ver PDF</Button>
            </>
          )}
          {mode==="global" && (
            <>
          ...
          {mode==="inventario" && <Button disabled>Descargar XLSX</Button>}
        </div>
      </Card>

      {/* Contenido condicionado por mode — ahora sí cambia */}
      {mode==="resumen" && (
        <Card>
          <p className="text-sm text-gray-500 mb-2">
            Aquí irá la bitácora personal (GET <code>/history/my-usage</code>) filtrada por fecha/tipo.
          </p>
          <Table>
            <Table.Head>
            ...
            <Table.Body className="divide-y">{/* data */}</Table.Body>
          </Table>
        </Card>
      )}
      ...
      {mode==="global" && (
        <Card>
          <p className="text-sm text-gray-500 mb-2">
            Reporte de uso global (reservas/préstamos/mantenimientos por periodo académico).
          </p>
          <Table>
            <Table.Head>
